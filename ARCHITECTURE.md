...
- **`!` prefix** and **`@Bot` mention** — both are text commands sharing one parser, `Context`, checks, and cooldowns. (`help` stays text-only — it's discord.py's built-in, not a hybrid command.) The mention path took a custom `command_prefix` (`_command_prefix` in [src/bot.py](src/bot.py)) plus one Bot flag to actually work — two behaviors that only surface against a live gateway, both regression-tested in `TestMentionPrefix`:
  - **`strip_after_prefix=True` is mandatory.** Discord sends `@Bot start` as `<@id>  start` — the mention pill carries its own trailing space *plus* the user's typed space. discord.py's default (`False`) strips only the single space baked into the matched prefix, leaving an **empty command name → `CommandNotFound` → silent no-op**. `True` skips the remaining whitespace so the command parses. (No effect on `!start`.)
  - **The bot's managed *role* mention is accepted, not just the user mention.** Adding a bot auto-creates a managed role with the bot's name, and typing `@BotName` usually resolves to that **role** (`<@&role_id>`), not the bot **user** (`<@user_id>`) — visually identical in the picker. `when_mentioned` only covers the user mention, so `_command_prefix` appends the bot's own `is_bot_managed()` role mention from `guild.me.roles`, making either autocomplete choice work.
...
- **No `*args`.** Slash commands need a declared parameter schema, so `stop`/`restart` expose explicit `container: str = None, now: str = None` params and **reconstruct the free-form token tuple** that `_delayed_container_op` parses. This preserves every text ordering (`!stop`, `!stop now`, `!stop <c>`, `!stop <c> now`, `!stop now <c>`) while giving slash users labeled fields. `now` is deliberately a **string, not a bool**: a bool param would make discord.py's converter reject the literal token `now` on the text path (e.g. `!stop server1 now`), breaking backward compatibility. To keep that from forcing slash users to *type* `now` into a field already labelled `now` (and to stop a plausible guess like `yes`/`0` from falling through to the container-name branch and erroring as an unknown container), the param carries `@app_commands.choices(now=...)` with a single choice whose **value stays exactly `now`** — the token `_delayed_container_op` matches on. Choices constrain only the application command; `clean_params["now"].converter` remains a plain `str`, so every text ordering is untouched. `Literal["now"]` would render the same dropdown but is **not** interchangeable: ext.commands treats `Literal` as a validating converter on the text path, which breaks the `!stop now <container>` ordering.
...
- **Detect the slash path via `ctx.interaction`, not `ctx.message`.** A hybrid slash `Context` carries a *synthetic* `ctx.message` that is non-`None` but has empty `.content` — so a `ctx.message`-truthiness check misclassifies slash as text. `on_command` logs `f"/{ctx.command} (slash)" if ctx.interaction is not None else ctx.message.content`. (The `CommandNotFound` branch's old `ctx.message.content` read only ever runs on the text path, and now uses `ctx.invoked_with` anyway.)
