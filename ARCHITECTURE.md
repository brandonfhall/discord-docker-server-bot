...
- `!start` branches on `container_health` right after a successful `start_container()`: `None` (no healthcheck defined) reports success immediately, same as always. A configured healthcheck instead sends an interim "started -- waiting for healthcheck" message and hands off to `_wait_for_healthy`, a `bot.loop.create_task()` background task (the same fire-and-forget pattern `_delayed_container_op` uses for stop/restart countdowns, so the command handler itself returns promptly rather than blocking command dispatch for however long the healthcheck takes). It polls every `HEALTHCHECK_POLL_INTERVAL` seconds until health leaves `starting` (`healthy`/`unhealthy` reported directly) or `HEALTHCHECK_MAX_WAIT` elapses (`0` disables the cap), and only then does the bot consider the container genuinely "started." A concurrent second `!start` while this is in flight just hits `start_container()`'s existing `"already running"` short-circuit — no separate dedup/`pending_ops` entry needed, unlike stop/restart, since starting an already-running container is naturally idempotent.
